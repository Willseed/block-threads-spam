...
1. `wrangler versions upload` 無法建立全新的 Worker 名稱，因此目標帳戶必須先存在 `threads-variant-guard` Worker 記錄。此帳戶已於 2026-07-20 建立不含 binding 或 runtime secret 的初始 Worker；其他帳戶不可假設 staged workflow 會代為建立。
2. 先確認上述九個 Actions secrets 全部存在，再啟動 workflow。runner 以 `umask 077` 建立 0600 暫存 secrets file，job 無論成功或失敗都會清除。
3. `wrangler versions upload` 會以包含 commit SHA、run ID 與 retry attempt 的唯一 tag 上傳尚未接收流量的 Worker 版本，同時把七個 runtime secrets 納入該版本，並依 `DB` 與 `EVIDENCE` binding 自動建立遠端 D1 database 與私有 R2 bucket。
4. 同一個 job 接著執行 `wrangler d1 migrations apply DB --remote`。`DB` 是 binding 名稱；Wrangler 會以既定的 automatic-provisioning 名稱解析同一個 database，不需在 repo 保存帳戶 UUID。
5. migration 成功後才以該次唯一 tag 執行 `wrangler versions deploy`，讓新版本接收 100% 流量。若 upload 或 migration 失敗，既有部署維持不變；修復後可安全重跑，Wrangler 只會套用尚未完成的 migration。

GitHub runner 是暫時環境，automatic provisioning 產生的資源識別不會回寫到 repository；遠端 binding 關係與實際資源名稱應在 Cloudflare dashboard 查核。Custom domain 與 Cloudflare Access Application 仍由 Cloudflare dashboard 管理。
...
Cloudflare Zero Trust 應建立較精確的 `spam.buy2330.cc/meta/threads/*` Self-hosted Application，政策設為 `Bypass` / `Everyone`。不要 bypass 整個 hostname，也不要把 `https://spam.buy2330.cc/auth/threads/callback` 納入；一般 OAuth callback 與所有 `/api/*` 仍必須通過 Access。
...
Wrangler 設定使用 automatic provisioning：首次 `versions upload` 時會為只宣告 binding 的 `DB` 與 `EVIDENCE` 建立遠端資源；CI 的暫時 checkout 不會把資源識別回寫到 repo，因此 repo 不保存帳戶專屬或虛構 UUID。正式部署前仍需逐一設定 `secrets.required`，並保持所有外部平台 feature flag 預設為 `false`。
