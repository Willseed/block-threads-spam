...
Cloudflare 的 staged upload 需要 Worker service record 已存在；`versions upload` 不能建立全新的 Worker。此專案已在 dashboard 一次性建立無 binding 的 Hello World `threads-variant-guard` 作為 bootstrap。之後在這個既有 Worker 上執行 version upload 時，automatic provisioning 可建立／綁定 `threads-variant-guard-db` 與 `threads-variant-guard-evidence`。這個 bootstrap 不代表 production version 已正式發布，也不代表 custom domain 已掛載。
