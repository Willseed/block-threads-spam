...
`versions upload` 不會建立全新 Worker service record，因此 Cloudflare dashboard 已一次性建立無 binding 的 Hello World `threads-variant-guard` 作為 bootstrap；此步驟已完成。既有 Worker 可在 staged upload 時 automatic provision `threads-variant-guard-db` D1 database 與 `threads-variant-guard-evidence` R2 bucket。dashboard 目前在此 Worker 顯示 custom domain `—`，表示 `spam.buy2330.cc` 尚未掛到該 Worker；正式部署後仍須驗證 domain mapping、TLS、Access path policy 與 callback reachability。
...
| GitHub Actions → Cloudflare API | 無 secrets 的 verify job 成功、既有 Worker bootstrap、main-only deploy job、exact-main `production` environment、最小權限帳戶 token | 供應鏈竄改、token 外洩、錯誤帳戶部署、同 commit rerun tag 混淆或錯誤 version 啟用 | workflow 最小權限、環境核准、固定 action 版本、run-unique tag activation、單一帳戶資源範圍、`0600` secrets file、always cleanup、機密遮罩與輪替 |
