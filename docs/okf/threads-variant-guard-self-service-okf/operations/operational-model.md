...
* Access team origin 與主 Application audience 以版本化 Worker vars 發布；五項剩餘 runtime 值只在 deploy job 寫入 runner 暫存目錄中的 `0600` secrets file，供 `wrangler versions upload` 建立未啟用 version。部署 token 不成為 runtime binding，暫存檔由 `always()` cleanup 移除。
